    ...
    public static class QuicksaveManager
    {
        ...
        private static string QuicksaveNamePattern
        {
            get
            {
                ...
                string saveSuffix;
                if (Config.UseDateTimeSuffix)
                    saveSuffix = Config.MaxQuicksaves > 1 ? @"(\d{8}-\d{6})" : string.Empty;
                else
                    saveSuffix = Config.MaxQuicksaves > 1 ? @"(\d{3})" : string.Empty;

                return $"^{playerCharacterName}{prefix}{saveSuffix}$";
            }
        }
        ...
        public static string GetNextQuicksaveName()
        {
            ...
            if (Config.UseDateTimeSuffix)
            {
                ...
                saveSuffix = $"{ts.Year:0000}" + $"{ts.Month:00}" + $"{ts.Day:00}" + $"-{ts.Hour:00}" + $"{ts.Minute:00}" + $"{ts.Second:00}"; // + $"-{ts.Millisecond:000}";
            }
            else
                saveSuffix = Config.MultipleQuicksaves ? $"{NextQuicksaveNumber:000}" : string.Empty;

            return $"{characterName}{Config.QuicksavePrefix}{saveSuffix}";
        }
